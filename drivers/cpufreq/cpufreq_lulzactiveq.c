...
static void cpufreq_lulzactive_timer(unsigned long data)
{
        ...
        if (dbs_tuners_ins.dvfs_debug) {
            if (pcpu->policy->cur < pcpu->policy->max) {
                printk(KERN_ERR "[PUMP UP] %s, CPU %d, %d>=%lu, from %d to %d\n",
                        __func__, pcpu->cpu, cpu_load, dbs_tuners_ins.inc_cpu_load, pcpu->policy->cur, new_freq);
            }
        }
    }
       ...
       if (dbs_tuners_ins.dvfs_debug) {
           if (pcpu->policy->cur > pcpu->policy->min) {
              printk(KERN_ERR "[PUMP DOWN] %s, CPU %d, %d<=%lu, from %d to %d\n",
                     __func__, pcpu->cpu, cpu_load, dbs_tuners_ins.dec_cpu_load, pcpu->policy->cur, new_freq);
            }
       }
    }
    else
        ...
        if (cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time)
            ...
            if (dbs_tuners_ins.dvfs_debug)  {
                printk (KERN_ERR "[PUMP REARM UP]: CPU %d, (%llu - %llu) < %lu\n",
                        pcpu->cpu, pcpu->timer_run_time, pcpu->freq_change_time, dbs_tuners_ins.up_sample_time);
            }
    ...
    if (new_freq < pcpu->target_freq) {
        ...
        if (dbs_tuners_ins.dvfs_debug) {
            printk (KERN_ERR "[PUMP DOWN NOW] CPU %d, after %u (%llu - %llu)\n", 
                    pcpu->cpu, (unsigned int) cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time),
        ...
        if (dbs_tuners_ins.dvfs_debug) {
            printk (KERN_ERR "[PUMP UP NOW] CPU %d, after %u (%llu - %llu)\n", 
                    pcpu->cpu, (unsigned int) cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time),
                    pcpu->timer_run_time, pcpu->freq_change_time);
        }
...
static int __init cpufreq_lulzactive_init(void)
{
    ...
    lulz_wq = alloc_workqueue("klulzactiveq_wq", 0, 0);
