...
#define LULZACTIVE_AUTHOR   "tegrak-robertobsc"
...
static struct workqueue_struct *lulz_wq;
...
static struct dbs_tuners {
    unsigned long up_sample_time;
    unsigned long down_sample_time;
    unsigned int  hotplug_sample_rate;
    unsigned long inc_cpu_load;
    unsigned long dec_cpu_load;
    unsigned long pump_up_step;
    unsigned long pump_down_step;
    unsigned long screen_off_min_step;
    ...
    unsigned int dvfs_debug;
        ...
        .hotplug_sample_rate=DEF_SAMPLING_RATE,
...
/*
 *
 *  Includes from pegasusQ governor with hotplug logic
 *
 */
...
static void apply_hotplug_lock(void)
{
    ...
    queue_work_on(dbs_info->cpu, lulz_wq, work);
}
...
/*
 * Queue Process functions for hotpluging second or more cores - begin
 */
...
struct cpu_usage_history *hotplug_lulzq_history;
...
{
    int num_hist = hotplug_lulzq_history->num_hist;
    struct cpu_usage *usage;
    int freq, rq_avg;
    int i;
    ...
    if (dbs_tuners_ins.dvfs_debug) printk(KERN_ERR "[HOTPLUG CHECK UP]: %d\n", num_hist);

    if(num_hist == 0) num_hist = MAX_HOTPLUG_RATE;

    for (i = num_hist - 1; i >= num_hist - up_rate; --i) {
        usage = &hotplug_lulzq_history->usage[i];
    ...
    if (avg_freq >= up_freq && avg_rq > up_rq) {
        printk(KERN_ERR "[HOTPLUG LULZ IN] %s %d>=%d && %d>%d\n",
    ...
    if (dbs_tuners_ins.dvfs_debug) {
        if (avg_freq >= up_freq) {
            ...
            printk(KERN_ERR "[NO HOTPLUG LULZ IN] %s %d>=%d BUT %d<=%d\n",
                       __func__, avg_freq, up_freq, avg_rq, up_rq);
...
static int check_down(void)
{
    ...
    if (dbs_tuners_ins.dvfs_debug) printk(KERN_ERR "[HOTPLUG CHECK DOWN]: %d\n", num_hist);
    ...
    if (avg_freq <= down_freq && avg_rq <= down_rq) {
        printk(KERN_ERR "[HOTPLUG LULZ OUT] %s %d<=%d && %d<%d\n",
    ...
    if (dbs_tuners_ins.dvfs_debug) {
        if (avg_freq <= down_freq) {

            printk(KERN_ERR "[NO HOTPLUG LULZ OUT] %s %d<=%d BUT %d>%d\n",
                       __func__, avg_freq, down_freq, avg_rq, down_rq);
        }
    }
...
static void check_hotplug(struct cpufreq_lulzactive_cpuinfo *pcpu)
{
    struct cpufreq_policy *policy;
    int num_hist = hotplug_lulzq_history->num_hist;
    ...
    hotplug_lulzq_history->usage[num_hist].freq = policy->cur;
    ++hotplug_lulzq_history->num_hist;

    hotplug_lulzq_history->usage[num_hist].rq_avg = get_nr_run_avg();

    /* Check for CPU hotplug */
    if (check_up())
    {
        queue_work_on(pcpu->cpu, lulz_wq, &pcpu->up_work);
    }
    else if (check_down())
    {
        queue_work_on(pcpu->cpu, lulz_wq, &pcpu->down_work);
    }

    if (hotplug_lulzq_history->num_hist  == max_hotplug_rate)
        hotplug_lulzq_history->num_hist = 0;
}
...
static inline void fix_screen_off_min_step(struct cpufreq_lulzactive_cpuinfo *pcpu) {
    if (pcpu->lulzfreq_table_size <= 0) {
        dbs_tuners_ins.screen_off_min_step = 0;
        return;
    }
    
    if (DEFAULT_SCREEN_OFF_MIN_STEP == dbs_tuners_ins.screen_off_min_step)
        for(dbs_tuners_ins.screen_off_min_step=0;
        pcpu->lulzfreq_table[dbs_tuners_ins.screen_off_min_step].frequency != 500000;
        dbs_tuners_ins.screen_off_min_step++);
    
    if (dbs_tuners_ins.screen_off_min_step >= pcpu->lulzfreq_table_size)
        for(dbs_tuners_ins.screen_off_min_step=0;
        pcpu->lulzfreq_table[dbs_tuners_ins.screen_off_min_step].frequency != 500000;
        dbs_tuners_ins.screen_off_min_step++);
}
...
static inline unsigned int adjust_screen_off_freq(
    ...
    if (early_suspended && freq > pcpu->lulzfreq_table[dbs_tuners_ins.screen_off_min_step].frequency) {
        freq = pcpu->lulzfreq_table[dbs_tuners_ins.screen_off_min_step].frequency;
...
static void cpufreq_lulzactive_timer(unsigned long data)
{
    ...
    if (dbs_tuners_ins.dec_cpu_load > dbs_tuners_ins.inc_cpu_load) {
    ...
    if (cpu_load >= dbs_tuners_ins.inc_cpu_load) {
        if (dbs_tuners_ins.pump_up_step && pcpu->policy->cur < pcpu->policy->max) {
            ...
            pcpu->cur_index -= dbs_tuners_ins.pump_up_step;
            if (pcpu->cur_index < 0)
                pcpu->cur_index = 0;
            
            new_freq = pcpu->lulzfreq_table[pcpu->cur_index].frequency;
        }
        else {
        ...
        if (dbs_tuners_ins.dvfs_debug) {
            if (pcpu->policy->cur < pcpu->policy->max) {
                printk(KERN_ERR "[PUMP UP] %s %d>=%lu, from %d to %d\n",
                        __func__, cpu_load, dbs_tuners_ins.inc_cpu_load, pcpu->policy->cur, new_freq);
            }
        }
    }
    else if (cpu_load <= dbs_tuners_ins.dec_cpu_load)
    {
        if (dbs_tuners_ins.pump_down_step) {
            ret = cpufreq_frequency_table_target(
                pcpu->policy, pcpu->lulzfreq_table,
                pcpu->policy->cur, CPUFREQ_RELATION_H,
                &pcpu->cur_index);
            ...
            pcpu->cur_index += dbs_tuners_ins.pump_down_step;
            if (pcpu->cur_index >= pcpu->lulzfreq_table_size) {
                pcpu->cur_index = pcpu->lulzfreq_table_size - 1;
            }
            
            new_freq = (pcpu->policy->cur > pcpu->policy->min) ? 
                (pcpu->lulzfreq_table[pcpu->cur_index].frequency) :
                (pcpu->policy->min);
        }
        else {
       ...
       if (dbs_tuners_ins.dvfs_debug) {
           if (pcpu->policy->cur > pcpu->policy->min) {
              printk(KERN_ERR "[PUMP DOWN] %s %d<=%lu, from %d to %d\n",
                     __func__, cpu_load, dbs_tuners_ins.dec_cpu_load, pcpu->policy->cur, new_freq);
            }
       }
    }
    else
    {
        new_freq = pcpu->policy->cur; //pcpu->lulzfreq_table[pcpu->cur_index].frequency;
    }
    ...
    if (new_freq < pcpu->target_freq) {
        if (cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time)
            < dbs_tuners_ins.down_sample_time) {

            if (dbs_tuners_ins.dvfs_debug) {
                printk (KERN_ERR "[PUMP REARM DOWN]: (%llu - %llu) < %lu\n",
                        pcpu->timer_run_time, pcpu->freq_change_time, dbs_tuners_ins.down_sample_time);
            }
        ...
        if (cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time)
            < dbs_tuners_ins.up_sample_time) {

            if (dbs_tuners_ins.dvfs_debug)  {
                printk (KERN_ERR "[PUMP REARM UP]: (%llu - %llu) < %lu\n",
                        pcpu->timer_run_time, pcpu->freq_change_time, dbs_tuners_ins.up_sample_time);
            }
    ...
    if (new_freq < pcpu->target_freq) {
        ...
        if (dbs_tuners_ins.dvfs_debug) {
            printk (KERN_ERR "[PUMP DOWN NOW] after %u (%llu - %llu)\n", 
                    (unsigned int) cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time),
        ...
        queue_work(lulz_wq, &freq_scale_down_work);
    } else {

        if (dbs_tuners_ins.dvfs_debug) {
            printk (KERN_ERR "[PUMP UP NOW] after %u (%llu - %llu)\n", 
                    (unsigned int) cputime64_sub(pcpu->timer_run_time, pcpu->freq_change_time),
                    pcpu->timer_run_time, pcpu->freq_change_time);
        }
...
static int cpufreq_lulzactive_up_task(void *data)
{
    ...
    while (1) {
		...
		pcpu->freq_change_time_in_idle =
			get_cpu_idle_time_us(j, &pjcpu->freq_change_time);
            }
...
{
    return sprintf(buf, "%lu\n", dbs_tuners_ins.inc_cpu_load);
}

static ssize_t store_inc_cpu_load(struct kobject *kobj,
            struct attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.inc_cpu_load)==-EINVAL) return -EINVAL;
    
    if (dbs_tuners_ins.inc_cpu_load > 100) {
        dbs_tuners_ins.inc_cpu_load = 100;
    }
    else if (dbs_tuners_ins.inc_cpu_load < 10) {
        dbs_tuners_ins.inc_cpu_load = 10;
    }
    return count;
}
...
static ssize_t show_dec_cpu_load(struct kobject *kobj,
                     struct attribute *attr, char *buf)
{
    return sprintf(buf, "%lu\n", dbs_tuners_ins.dec_cpu_load);
}

static ssize_t store_dec_cpu_load(struct kobject *kobj,
            struct attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.dec_cpu_load)==-EINVAL) return -EINVAL;

    if (dbs_tuners_ins.dec_cpu_load > 90) {
        dbs_tuners_ins.dec_cpu_load = 90;
    }
    else if (dbs_tuners_ins.dec_cpu_load < 0) {
        dbs_tuners_ins.dec_cpu_load = 10;
    }
...
static ssize_t store_down_sample_time(struct kobject *kobj,
            struct attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.down_sample_time)==-EINVAL) return -EINVAL;
    return count;
}
...
static ssize_t store_up_sample_time(struct kobject *kobj,
            struct attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.up_sample_time)==-EINVAL) return -EINVAL;
    return count;
}
...
static ssize_t show_pump_up_step(struct kobject *kobj,
                     struct attribute *attr, char *buf)
{
    return sprintf(buf, "%lu\n", dbs_tuners_ins.pump_up_step);
}

static ssize_t store_pump_up_step(struct kobject *kobj,
            struct attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.pump_up_step)==-EINVAL) return -EINVAL;
    return count;
}
            ...
            struct attribute *attr, const char *buf, size_t count)
{
    ...
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.pump_down_step)==-EINVAL) return -EINVAL;
    
    pcpu = &per_cpu(cpuinfo, 0);
    // fix out of bound
    if (pcpu->lulzfreq_table_size <= dbs_tuners_ins.pump_down_step) {
        dbs_tuners_ins.pump_down_step = pcpu->lulzfreq_table_size - 1;
    }
    return count;
}
            ...
            struct attribute *attr, const char *buf, size_t count)
{
    ...
    if(strict_strtoul(buf, 0, &dbs_tuners_ins.screen_off_min_step)==-EINVAL) return -EINVAL;
...
// define parameter file names and functions.

define_one_global_rw(inc_cpu_load);
define_one_global_rw(dec_cpu_load);
define_one_global_rw(down_sample_time);
define_one_global_rw(up_sample_time);
define_one_global_rw(debug_mode);
define_one_global_rw(pump_up_step);
define_one_global_rw(pump_down_step);
define_one_global_rw(screen_off_min_step);
define_one_global_ro(author);
define_one_global_ro(tuner);
define_one_global_ro(version);
define_one_global_ro(freq_table);

// Parameters From PegasusQ governor.
...
static ssize_t store_hotplug_sample_rate(struct kobject *a, struct attribute *b,
                 const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    if (ret != 1)
        return -EINVAL;
    dbs_tuners_ins.hotplug_sample_rate = input; //, MIN_SAMPLING_RATE);
    return count;
}
...
show_one(hotplug_sample_rate,hotplug_sample_rate);
...
define_one_global_rw(hotplug_sample_rate);
...
static struct attribute *lulzactive_attributes[] = {
    ...
    &dec_cpu_load.attr,
    &up_sample_time.attr,
    &down_sample_time.attr,
    &pump_up_step.attr,
    &pump_down_step.attr,
    &screen_off_min_step.attr,
    &debug_mode.attr,

    /*hotplug attributes*/

    &hotplug_sample_rate.attr,
...
static void do_hot_timer(struct work_struct *work)
{
    struct cpufreq_lulzactive_cpuinfo *pcpu =
            container_of(work, struct cpufreq_lulzactive_cpuinfo, work.work);

    unsigned int cpu = pcpu->cpu;
    int delay;
     ...
     * same jiffy
     */
    delay = usecs_to_jiffies(dbs_tuners_ins.hotplug_sample_rate);
...
static inline void hotplug_timer_init(struct cpufreq_lulzactive_cpuinfo *pcpu)
{
    /* We want all CPUs to do sampling nearly on same jiffy */

    int delay = usecs_to_jiffies(DEF_START_DELAY * 1000 * 1000
            + dbs_tuners_ins.hotplug_sample_rate);

    if (num_online_cpus() > 1)
        delay -= jiffies % delay;

    INIT_DELAYED_WORK_DEFERRABLE(&pcpu->work, do_hot_timer);
    INIT_WORK(&pcpu->up_work, cpu_up_work);
    INIT_WORK(&pcpu->down_work, cpu_down_work);

    queue_delayed_work_on(pcpu->cpu, lulz_wq,
            &pcpu->work, delay + 2 * HZ);
}
...
void start_lulzactiveq(void)
{
    ...
    if( dbs_tuners_ins.pump_up_step == 0 )
    {
        pcpu = &per_cpu(cpuinfo, 0);
...
void stop_lulzactiveq(void)
{
    ...
    dbs_tuners_ins.pump_up_step = DEFAULT_PUMP_UP_STEP;
    dbs_tuners_ins.pump_down_step = DEFAULT_PUMP_DOWN_STEP;
...
static int __init cpufreq_lulzactive_init(void)
{
    ...
    hotplug_lulzq_history = kzalloc(sizeof(struct cpu_usage_history), GFP_KERNEL);
    if (!hotplug_lulzq_history) {
    ...
    lulz_wq = alloc_workqueue("klulzactiveq_wq", 0, 1);
...
MODULE_AUTHOR("robertobsc - gokhanmoral");
