...
static void mif_log2str(struct mif_event_buff *evtb, char *buff)
{
	...
	sprintf(evtb->time, "%02d-%02d %02d:%02d:%02d.%03ld",
		(1 + date.tm_mon), date.tm_mday,
		date.tm_hour, date.tm_min, date.tm_sec,
		(tv->tv_usec / 1000));

	if (evtb->evt == MIF_IRQ_EVT) {
		sprintf(buff, "%s IRQ <%s:%s> ",
			evtb->time, evtb->iod, evtb->ld);
		mif_irq2str(evtb, buff);
		mif_fin_str(buff);
	} else if (evtb->evt == MIF_URG_EVT) {
		sprintf(buff, "%s URG <%s:%s> %s",
			evtb->time, evtb->iod, evtb->ld, evtb->data);
	} else if (evtb->evt == MIF_ERR_EVT) {
		sprintf(buff, "%s ERR <%s:%s> %s",
			evtb->time, evtb->iod, evtb->ld, evtb->data);
	} else if (evtb->evt == MIF_LOG_EVT) {
		sprintf(buff, "%s LOG <%s:%s> %s",
			evtb->time, evtb->iod, evtb->ld, evtb->data);
	} else if (evtb->evt == MIF_DBG_EVT) {
		sprintf(buff, "%s DBG <%s:%s> %s",
			evtb->time, evtb->iod, evtb->ld, evtb->data);
	} else if (evtb->evt >= MIF_LNK_RX_EVT || evtb->evt <= MIF_LNK_TX_EVT) {
...
void mif_event(enum mif_event_id evt, struct modem_ctl *mc,
	struct io_device *iod, struct link_device *ld, u8 *buff, unsigned size)
{
	struct sk_buff *skb;
	...
	if (evt > MIF_LNK_TX_EVT && evt < MAX_MIF_EVT)
		len = min_t(unsigned, MAX_MIF_STR_LEN, size);
	else
		len = min_t(unsigned, MAX_MIF_LOG_LEN, size);

	skb = alloc_skb(MAX_MIF_EVT_BUFF_SIZE, GFP_ATOMIC);
	if (skb) {
		skb_put(skb, MAX_MIF_EVT_BUFF_SIZE);
		memset(skb->data, 0, MAX_MIF_EVT_BUFF_SIZE);
		...
		evtb->rcvd = size;
		evtb->len = len;
		memcpy(evtb->data, buff, len);

		skb_queue_tail(&mc->evtq, skb);
	} else {
		if (evt == MIF_LOG_EVT || evt == MIF_ERR_EVT)
			printk(KERN_ERR "%s", buff);
		else if (evt == MIF_URG_EVT)
			printk(KERN_ALERT "%s", buff);
		else
			mif_print_data(buff, len);
	}
}
...
static void mif_evt_work(struct work_struct *work)
{
	...
	if (fp) {
		...
		if (size > MAX_MIF_LOG_FILE_SIZE) {
			mif_urg(mc, "<%s> %s size %lld > %d\n",
				__func__, mc->log_path, size,
