...
static int midas_wm1811_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = snd_jack_set_key(wm1811->jack.jack, SND_JACK_BTN_0, KEY_SEND);
	if (ret < 0)
		dev_err(codec->dev, "Failed to set KEY_SEND: %d\n", ret);
...
static struct snd_soc_dai_link midas_dai[] = {
	...
	{
/* Dummy DAI i/f for slp platform, it will be removed  */
		.name = "WM8994 AIF1 dummy",
		.stream_name = "Pri_Dai",
		.cpu_dai_name = "samsung-i2s.0",
		.codec_dai_name = "wm8994-aif1",
		.platform_name = "samsung-audio",
