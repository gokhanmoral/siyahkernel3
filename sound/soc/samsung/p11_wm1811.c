...
static bool p11_fll1_active;

static void p11_set_mclk(bool on)
{
	u32 val;
...
static void p11_start_fll1(struct snd_soc_dai *aif1_dai)
{
	int ret;
...
static int p11_wm1811_aif1_hw_params(struct snd_pcm_substream *substream,
...
/*
 * P11 WM1811 DAI operations.
 */
static struct snd_soc_ops p11_wm1811_aif1_ops = {
	.hw_params = p11_wm1811_aif1_hw_params,
};

static int p11_wm1811_aif2_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static struct snd_soc_ops p11_wm1811_aif2_ops = {
	.hw_params = p11_wm1811_aif2_hw_params,
};

static int p11_wm1811_aif3_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *params)
{
	pr_err("%s: enter\n", __func__);
	return 0;
...
static struct snd_soc_ops p11_wm1811_aif3_ops = {
	.hw_params = p11_wm1811_aif3_hw_params,
};
...
static const struct snd_kcontrol_new p11_controls[] = {
...
const struct snd_soc_dapm_widget p11_dapm_widgets[] = {
...
const struct snd_soc_dapm_route p11_dapm_routes[] = {
	{ "HP", NULL, "HPOUT1L" },
	{ "HP", NULL, "HPOUT1R" },
...
static struct snd_soc_dai_driver p11_ext_dai[] = {
	{
		.name = "p11.cp",
		.playback = {
			.channels_min = 1,
...
static int p11_wm1811_init_paiftx(struct snd_soc_pcm_runtime *rtd)
{
#if 0
	...
	ret = snd_soc_add_controls(codec, p11_controls,
					ARRAY_SIZE(p11_controls));

	ret = snd_soc_dapm_new_controls(&codec->dapm, p11_dapm_widgets,
					   ARRAY_SIZE(p11_dapm_widgets));
	if (ret != 0)
		dev_err(codec->dev, "Failed to add DAPM widgets: %d\n", ret);

	ret = snd_soc_dapm_add_routes(&codec->dapm, p11_dapm_routes,
					   ARRAY_SIZE(p11_dapm_routes));
	if (ret != 0)
	...
	ret = snd_soc_jack_new(codec, "P11 Jack",
...
static struct snd_soc_dai_link p11_dai[] = {
	{ /* Sec_Fifo DAI i/f */
		.name = "Sec_FIFO TX",
		.stream_name = "Sec_Dai",
		...
		.init = p11_wm1811_init_paiftx,
		.ops = &p11_wm1811_aif1_ops,
	},
	{
		.name = "P11_WM1811 Voice",
		.stream_name = "Voice Tx/Rx",
		.cpu_dai_name = "p11.cp",
		.codec_dai_name = "wm8994-aif2",
		.platform_name = "snd-soc-dummy",
		.codec_name = "wm8994-codec",
		.ops = &p11_wm1811_aif2_ops,
	},
	{
		.name = "P11_WM1811 BT",
...
#if 0	/* To Do */
static int p11_set_bias_level(struct snd_soc_card *card,
...
static int p11_set_bias_level_post(struct snd_soc_card *card,
					 enum snd_soc_bias_level level)
{
	int ret;
...
static struct snd_soc_card p11 = {
	.name = "P11_WM1811",
	.dai_link = p11_dai,

	/* If you want to use sec_fifo device,
	 * changes the num_link = 2 or ARRAY_SIZE(p11_dai). */
	.num_links = ARRAY_SIZE(p11_dai),

#if 0	/* To Do */
	.set_bias_level = p11_set_bias_level,
	.set_bias_level_post = p11_set_bias_level_post
#endif
};

static struct platform_device *p11_snd_device;

static int __init p11_audio_init(void)
{
	int ret;
	...
	ret = snd_soc_register_dais(&p11_snd_device->dev,
					p11_ext_dai, ARRAY_SIZE(p11_ext_dai));
	if (ret != 0)
		pr_err("Failed to register external DAIs: %d\n", ret);

	platform_set_drvdata(p11_snd_device, &p11);

	ret = platform_device_add(p11_snd_device);
	if (ret)
		platform_device_put(p11_snd_device);

	return ret;
}
module_init(p11_audio_init);

static void __exit p11_audio_exit(void)
{
	platform_device_unregister(p11_snd_device);
}
module_exit(p11_audio_exit);

MODULE_AUTHOR("JS. Park <aitdark.park@samsung.com>");
MODULE_DESCRIPTION("ALSA SoC P11 WM1811");
MODULE_LICENSE("GPL");
