...
static struct exynos_usb_phy usb_phy_control = {
	.phy_lock = __SPIN_LOCK_UNLOCKED(usb_phy_control.phy_lock)
};
...
static void exynos_usb_phy_control(enum usb_phy_type phy_type , int on)
{
	...
	if (soc_is_exynos4210()) {
		if (phy_type & USB_PHY0) {
			...
			else if ((usb_phy_control.phy0_usage > 0) &&
				on == PHY_DISABLE &&
		...
		if (phy_type & USB_PHY1) {
			...
			else if ((usb_phy_control.phy1_usage > 0) &&
				on == PHY_DISABLE &&
		...
		if (phy_type & USB_PHY_HSIC1) {
			...
			else if ((usb_phy_control.phy2_usage > 0) &&
				on == PHY_DISABLE &&
				(--usb_phy_control.phy2_usage) == 0)
...
static int exynos4_usb_dev_phy20_suspend(struct platform_device *pdev)
{
	u32 phypwr;

	/* set to suspend PHY0 */
	phypwr = readl(EXYNOS4_PHYPWR);
	phypwr |= PHY0_FORCE_SUSPEND;
	writel(phypwr, EXYNOS4_PHYPWR);

	return 0;
}

static int exynos4_usb_dev_phy20_resume(struct platform_device *pdev)
{
	u32 phypwr;
	/* unset SUSPEND to resume PHY0 */
	phypwr = readl(EXYNOS4_PHYPWR);
	phypwr &= ~(PHY0_FORCE_SUSPEND);
	writel(phypwr, EXYNOS4_PHYPWR);
...
static int exynos_usb_dev_phy20_init(struct platform_device *pdev)
{
	...
	exynos4_usb_dev_phy20_resume(pdev);
...
static int exynos_usb_dev_phy20_exit(struct platform_device *pdev)
{
	...
	exynos4_usb_dev_phy20_suspend(pdev);
