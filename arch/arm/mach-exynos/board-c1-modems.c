...
#if defined(CONFIG_MACH_C1VZW) && defined(CONFIG_CDMA_MODEM_CBP72)
	.use_handover = true,
#else
	.use_handover = false,
#endif
...
static int cmc_idpram_wakeup(void)
{
	...
	cnt = 0;
	while (1) {
		magic = cmc_idpram_get_magic();
		access = cmc_idpram_get_access();
		if (magic == DPRAM_MAGIC_CODE && access == 1)
			break;

		if (cnt++ > 10) {
			pr_err("\n[MDM/E] <%s> magic = 0x%X, access = 0x%X\n\n",
				__func__, magic, access);
			return -EAGAIN;
		}
...
#if defined(CONFIG_MACH_C1VZW) && defined(CONFIG_LTE_MODEM_CMC221)
	.use_handover = true,
#else
	.use_handover = false,
#endif
